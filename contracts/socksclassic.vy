...
# @dev Mapping from address to minter status.
minters: public(map(address, bool))

# @dev Address of URI contract
newTokenURI: public(URI)
...
def __init__():
    ...
    """
    self.name = 'Digital Unisocks Classic Edition 0'
    self.symbol = 'SOCKSCLASSIC'
...
def ownerOf(_tokenId: uint256) -> address:
    ...
    """
    _owner: address = self.idToOwner[_tokenId]
    # Throws if `_tokenId` is not a valid NFT
    assert _owner != ZERO_ADDRESS
    return _owner
...
def _isApprovedOrOwner(_spender: address, _tokenId: uint256) -> bool:
    ...
    """
    _owner: address = self.idToOwner[_tokenId]
    spenderIsOwner: bool = _owner == _spender
...
def approve(_approved: address, _tokenId: uint256):
    ...
    senderIsApprovedForAll: bool = (self.ownerToOperators[_owner])[msg.sender]
...
def addMinter(_minter: address):
    """
    @dev Adds address as a minter.
         Throws unless `msg.sender` is the current owner
    @param _minter Address of a minter to add.
    """
...
def removeMinter(_minter: address):
    """
    @dev Remove address from minters.
         Throws unless `msg.sender` is the current owner
    @param _minter Address of a minter to remove.
    """
    assert self.owner == msg.sender
...
@public
def setTokenURI(_tokenURI: address):
    """
    @dev Sets token uri contract address
         Throws unless `msg.sender` is the current owner
    @param _tokenURI Address of a contract to proxy call.
    """
    assert msg.sender == self.owner
    self.newTokenURI = URI(_tokenURI)
